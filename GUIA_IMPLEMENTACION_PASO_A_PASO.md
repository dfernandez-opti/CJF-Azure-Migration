...
Get-Content "CJF-Azure-Migration\.github\workflows\deploy-alz.yml" | Select-Object -First 20
```

### Paso 2.3: Ejecutar Primer Workflow (Validación)

1. Ve a tu repositorio en GitHub
2. Actions → Deploy Azure Landing Zone (ALZ) - CJF
...
8. Service connection name: `CJF-Azure-ServiceConnection`
9. Save
...
1. Pipelines → Library
2. + Variable group
3. Name: `CJF-Azure-DevOps-Variables`
4. Agregar variables:
...
.\deploy-alz.ps1 `
    -SubscriptionId "TU-SUBSCRIPTION-ID" `
    -Location "eastus" `
    -ResourceGroupName "rg-cjf-alz-deployment" `
    -RunALZReview
```
...
az group list --subscription "TU-SUBSCRIPTION-ID" --query "[?contains(name, 'cjf')]" --output table
```
...
2. Selecciona "Deploy Azure Landing Zone (ALZ) - CJF"
3. Run workflow
4. Ingresa:
   - Subscription ID
   - Environment: production
...
az monitor log-analytics workspace list --query "[?contains(name, 'cjf')]" --output table
...
# 4. Guarda en: CJF-Azure-Migration/3.0-Manage-Optimize/3.3-Operations-Management/monitoring-dashboards/
```
...
az automation account create `
    --name "aa-cjf-automation" `
    --resource-group "rg-cjf-alz-deployment" `
    --location "eastus" `
    --sku "Basic"
```
...
az backup vault list --query "[?contains(name, 'cjf')]" --output table
```
...
az group delete --name "rg-cjf-alz-deployment" --yes

# O usar un nombre diferente
.\deploy-alz.ps1 -ResourceGroupName "rg-cjf-alz-deployment-v2"
```
